...
use App\Http\Controllers\Controller as LaravelAppController;
use Illuminate\Http\Request;

use R\Lib\Form\FormRepositry;
use R\Lib\Http\InputValues;

class Controller extends LaravelAppController implements FormRepositry
{
    public function callAction($method, $parameters)
    {
        $this->request = \Request::getFacadeRoot();
        $this->route = \Route::getFacadeRoot();
...
// -- act実装向け機能
    ...
    {
        if (preg_match('!^id://(.*?)$!', $uri, $_)) {
            $url = app("laravelizer")->convertPageToUrl($_[1], $query_params, $fragment);
        } else {
            $url = $uri;
        }
        return redirect($url);
...
// -- Http系実装
    ...
    protected function makeResponse ()
    {
        ...
        return app("view")->file($file, $this->getTemplateVars());
    }
    protected function getTemplateFile ()
    {
        $path = $this->request->route()->uri();
        $file = app("laravelizer")->convertPathToFile($path);
        $file = app("laravelizer")->completeDirectoryIndex($file);
        return $file;
    }
