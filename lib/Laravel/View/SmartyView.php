...
// -- Include処理プラグイン
    ...
    public static function smarty_function_inc ($params, $smarty)
    {
        ...
        $file = app("laravelizer")->convertPathToFile($params["path"]);
        // if ($uri) {
...
// -- URL解決プラグイン
    ...
    {
        return app("laravelizer")->convertPageToUrl($page_id, $query_params, $anchor);
    }
    public static function smarty_modifier_path_to_url ($path, $url_params=array(), $anchor=null)
    {
        return app("laravelizer")->convertPathToUrl($path, $query_params, $anchor);
    }

// -- 認証解決プラグイン

    public static function smarty_modifier_is_accessible ($page)
    {
        return app("laravelizer")->getPageIsAccessible($page);
    ...
    {
        return app("laravelizer")->getPriv($role, $priv_id);
