...
class TableRelationResolver
{
    ...
    public function getFkeyRoute($from_table, $to_table)
    {
        ...
        $from_table_def = app()->tables[$from_table];
        // 明示的に指定された経路の探索
        foreach ((array)$from_table_def->getDefAttr("fkey_routes") as $assoc_table=>$route) {
        ...
        // 全ての外部キーを逐次探索
        foreach ($from_table_def->getDefAttr("cols") as $fkey_col_name=>$from_col_def) {
                ...
                if ( ! $this->fkey_routes[$from_table][$assoc_table]) {
                    ...
                    $this->fkey_routes[$from_table][$assoc_table] = array(
                        array(
                            ...
                            app()->tables[$assoc_table]->getIdColName(), // 関係先テーブル上の主キー
                            // "where"=>array(), // 追加条件
                            // "as"=>null, // Join時の別名
                        ),
                    );
