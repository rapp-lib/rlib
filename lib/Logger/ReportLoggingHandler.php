...
class ReportLoggingHandler extends AbstractProcessingHandler
{
     ...
     * レポートドライバ
     */
    private function report ($errstr, $params=null, $options=array())
    {
        // 旧report互換処理
    ...
    private function report_template ($errstr, $params, $options, $backtraces, $config)
    {
        ...
        if ($config["output_format"]=="html") {
            ...
            if (is_array($params) && is_string($errstr)) {
                ...
                $message .=$this->decorate_value($errstr,true);
            }
            if ($params!==null) {
            ...
            $report_html .='<div class="ruiReport '.$elm_class.'" id="'.$elm_id.'" '
                    ...
                    .'Backtrace: '.$this->decorate_value(array_reverse($errdetail),true).'</div></div>';
        // 非HTML形式
        } elseif ( ! $html_mode) {
            if (is_string($errstr)) {
                $report_html .= "*\n* ".$errstr."\n*";
            } else {
                $report_html .= "[VALUE] ".$this->decorate_value($errstr,false);
            }
            if (is_array($params)) {
                $report_html .= "\n[PARAM] ".$this->decorate_value($params,false);
            }
            if (is_array($errdetail)) {
                $report_html .="\n".'[TRACE] '.$this->decorate_value(array_reverse($errdetail),false);
            }
    ...
    private function decorate_value ($target_value, $html_mode=false, $level=1)
    {
        ...
        } elseif ($info =$this->report_profile_function($target_value)) {
            ...
            if (count($target_value)) {
                foreach ($target_value as $key => $value) {
                    $result .=str_repeat($sp_code,$level*3).'['.$key.']=>'
                            .$sp_code.$this->decorate_value($value,$html_mode,$level+1)
                            .$br_code;
                }
            }
