...
// -- 文基幹部
    ...
    {
        return "SELECT ".$this->stSelectFields($q["fields"])." FROM ".$this->stTable($q["table"])
    ...
    {
        return "DELETE ".$this->stTable($q["table"]).$this->stWhere($q["where"], "WHERE");
    }
// -- JOIN句
...
// -- WHERE句
    ...
    public function stCondItem($k, $v)
    {
        ...
        if (preg_match('!^(.+?)\s+([<>=]=?|<>|\\!=|LIKE|IN)$!i',$k,$_)) $this->stKvCrit($_[1], $_[2], $v);
    ...
    public function stKvCrit($k, $op, $v)
    {
        ...
        return $this->stName($k)." = ".$this->stValue($v);
    }
// -- SELECT文固有の表現
