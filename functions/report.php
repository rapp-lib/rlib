...
// -- 廃止予定
        ...
        $config=array())
    {
        ...
        // 非HTML形式
            ...
            if (is_string($errstr)) {
                ...
                $report_html .= "[VALUE] ".decorate_value($errstr,false);
            }
            if (is_array($params)) {
                $report_html .= "\n[PARAM] ".decorate_value($params,false);
            }
            if (is_array($errdetail)) {
                $report_html .="\n".'[TRACE] '.decorate_value(array_reverse($errdetail),false);
            }
    ...
    //-------------------------------------
    // レポートドライバ
    function report ($errstr, $params=null, $options=array())
    {
