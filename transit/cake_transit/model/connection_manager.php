 ...
 */
	public static function &getDataSource($name) {
		...
		if (empty($_this->_connectionsEnum[$name])) {
			trigger_error(sprintf(__esc("ConnectionManager::getDataSource - Non-existent data source %s", true), $name), E_USER_ERROR);
			$null = null;
			return $null;
		}
		...
		if ($_this->loadDataSource($name) === null) {
			trigger_error(sprintf(__esc("ConnectionManager::getDataSource - Could not load class %s", true), $class), E_USER_ERROR);
			$null = null;
			return $null;
		}
 ...
 */
	public static function loadDataSource($connName) {
		...
		if (!App::import('Datasource', $class, !is_null($conn['plugin']))) {
			trigger_error(sprintf(__esc('ConnectionManager::loadDataSource - Unable to import DataSource class %s', true), $class), E_USER_ERROR);
			return null;
		}
		return true;
	}
